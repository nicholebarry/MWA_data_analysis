PRO vis_res_seti_sepdays,save_point=save_point, quick_run=quick_run, even_odd=even_odd
...
;  paralleled=1
  ...
  if keyword_set(grid_eng) then begin
    compile_opt strictarr
    args = Command_Line_Args(count=nargs)
    obs_filename = args[0]
    outdir = args[1]
  ...
  if ~keyword_set(outdir) then outdir = '/nfs/eor-00/h1/nbarry/vis_res/sim_4000/'
  ;Read in the obsids
  if ~keyword_set(obs_filename) then obs_filename='/nfs/eor-00/h1/nbarry/MWA/IDL_code/obs_list/Aug23zenith.txt
  readcol, obs_filename, obsids, format='A', /silent
  if ~keyword_set(dir) then dir='/nfs/mwa-09/r1/djc/EoR2013/Aug23/fhd_nb_sim_overfit_cal_eor_ones_short_baselines_included_zenithpointing/'
