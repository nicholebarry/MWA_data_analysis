...
def main():
	folder_name = ['jd2456566v4_1', 'jd2456574v4_1', 'jd2456538v4_1',  'jd2456545v4_1',  'jd2456553v4_1',  'jd2456568v4_1',  'jd2456576v4_1',  'jd2456547v4_1',  'jd2456555v4_1',  'jd2456570v4_1', 'jd2456589v4_1']
	for fn in folder_name:
		folder_loop(fn)

def folder_loop(folder_name):
	...
	version=4
	subversion=1
	old_path = '/nfs/eor-14/r1/EoRuvfits/'
	old_path2 = '/nfs/mwa-14/r1/EoRuvfits/'
	...
	try:
		...
		obsfile_name='/nfs/mwa-00/h1/nbarry/MWA/IDL_code/obs_list/datafile_transfer4.txt'
		...
		obsids_path = glob.glob(old_path + '*.uvfits')
	...
	for obsid in obsids:
		...
		save_path = all_nodes[6] + save_directories[obs_index] + obsid #put approximately 500 obsids on specified node if following test turns up false
		...
		metafits_file = old_path + obsid + ".metafits"
		...
		cur.execute("UPDATE uvfits SET (path,cotter_version,bottom_freq_mhz,top_freq_mhz)=(%s,%s,%s,%s) WHERE (path,obsid)=(%s,%s);", \
			(new_path,cotter_version,bottom_freq_mhz,top_freq_mhz,old_path2 + obsid + ".uvfits",obsid))
		conn.commit()
