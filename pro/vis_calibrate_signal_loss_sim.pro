FUNCTION vis_calibrate,vis_ptr,cal,obs,status_str,psf,params,jones,flag_ptr=flag_ptr,model_uv_arr=model_uv_arr,$
    ...
    calibration_auto_fit=calibration_auto_fit,over_calibrate=over_calibrate,saved_run_std_test_polyquad=saved_run_std_test_polyquad,_Extra=extra
  t0_0=Systime(1)
  error=0
  timing=-1
  heap_gc
  ...
  IF Keyword_Set(calibration_polyfit) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      If keyword_set(saved_run_std_test_polyquad) then begin
        ...
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_std_test_nodigtwopolyquad/'+obs.obsname+'_cal.sav
