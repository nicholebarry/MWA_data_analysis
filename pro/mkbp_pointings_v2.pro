pro mkbp_pointings_v2, dir_name,day=day, advanced_plotting=advanced_plotting, longrun=longrun, split_jump=split_jump, $
  ...
  ;poly_saved_file=1
  If keyword_set(poly_saved_file) then gain_fit_restored=getvar_savefile('/nfs/eor-00/h1/nbarry/longrun_polyfit_v2.sav','gain_fit') ;pol x day x freq x tile x pointing
  ;If keyword_set(poly_saved_file) then gain_fit_restored2=getvar_savefile('/nfs/eor-00/h1/nbarry/longrun_polyfit_3.sav','gain_fit') ;pol x day x freq x tile x pointing
  ;If keyword_set(poly_saved_file) then begin
  ;  gain_fit_restored=FLTARR(2,16,384,128,6)
  ;  gain_fit_restored[*,*,*,*,0:4]=gain_fit_restored1
  ;;  gain_fit_restored[*,*,*,*,5]=gain_fit_restored2
  ;endif
  ...
  FOR j=0, (size(pointing_num))[1]-1 DO BEGIN
    ...
    phase_transfer_ptr=PTRARR(parsednumbers[j], /allocate) ;pointings x obs
    phase_transfer_orig_ptr=PTRARR(parsednumbers[j], /allocate) ;pointings x obs
    ...
    IF obsid NE 'empty' THEN BEGIN
      ...
      For i=0, parsednumbers[j]-1 DO BEGIN
        ...
        IF obsid NE 'empty' THEN BEGIN
          ...
          If keyword_set(longrun) then restore, '/nfs/eor-03/r1/EoR2013/fhd_apb_EoR0_high_sem1_1/calibration/' + obsid + '_cal.sav' else $
            ...
            ;restore, '/nfs/eor-00/h1/nbarry/Aug23_longrunpoly_onephase/'+ obsid +'_cal.sav'
          ...
          if keyword_set(phase_transfer) then begin
            ...
            *phase_transfer_ptr[i]=phase_transfer
            
          endif
          
          ;If the autos keyword is set, restore the cal structure from a regular run (Aug23 obsids). Only an option for nonlongruns
          If keyword_set(save_autos) and ~keyword_set(longrun) then restore, '/nfs/mwa-09/r1/djc/EoR2013/Aug23/fhd_nb_autogains_Aug2015/calibration/' + obsid + '_cal.sav'
          
          If keyword_set(save_autos) and keyword_set(longrun) then begin
            print, 'Autos only available for Aug23 and Aug27!'
            If day EQ 'Aug23' then restore, '/nfs/mwa-09/r1/djc/EoR2013/Aug23/fhd_nb_autogains_Aug2015/calibration/' + obsid + '_cal.sav'
          ...
          ;for pol_i=0,n_pol[i]-1 do *cal_array[i].gain[pol_i]=exp(Complex(0,1)*atan(*cal_array[i].gain[pol_i],/phase))
          endif else begin
            ...
            if keyword_set(phase_transfer) and keyword_set(compare) then begin
              if i eq 0 then cal_array_compare = replicate(cal2,parsednumbers[j]) else cal_array_compare[i]=cal2
              cal_array_compare[i].gain[0]=Pointer_copy(cal2.gain[0])
              cal_array_compare[i].gain[1]=Pointer_copy(cal2.gain[1])
            ;*cal_array_compare[i].gain[0]=(*cal_array[i].gain[0])*exp(Complex(0,1)*phase_transfer[0,*,*])
            ;*cal_array_compare[i].gain[1]=(*cal_array[i].gain[1])*exp(Complex(0,1)*phase_transfer[1,*,*])
            ...
            if keyword_set(phase_transfer) and keyword_set(compare) then begin
              ...
              restore,'/nfs/eor-00/h1/nbarry/compare_ratio_v2_'+strtrim(pointing_num[j],2)+'.sav'
              for freq_i=0,383 do begin
                (*cal_array[i].gain[0])[freq_i,*]=((*cal_array[i].gain[0])[freq_i,*]/compare_ratio[*,0]) *exp(Complex(0,1)*phase_transfer[0,freq_i,*])
                (*cal_array[i].gain[1])[freq_i,*]=((*cal_array[i].gain[1])[freq_i,*]/compare_ratio[*,1]) *exp(Complex(0,1)*phase_transfer[1,freq_i,*])
              endfor
              ...
              if keyword_set(compare) then begin
                restore,'/nfs/eor-00/h1/nbarry/compare_ratio_v2_'+strtrim(pointing_num[j],2)+'.sav'
                for freq_i=0,383 do begin
                  (*cal_array[i].gain[0])[freq_i,*]=((*cal_array[i].gain[0])[freq_i,*]/compare_ratio[*,0])
                  (*cal_array[i].gain[1])[freq_i,*]=((*cal_array[i].gain[1])[freq_i,*]/compare_ratio[*,1])
                endfor
              endif
            endelse
          ...
          cal_bandpass=vis_cal_bandpass(cal_array[i],obs_array[i],cal_remainder=cal_remainder,cable_bandpass_fit=1)
      ...
      ;get the average phase for the pointing per tile
      ;      phase_all=FLTARR(2,384,128,parsednumbers[j])
      ;      for obs_i=0, parsednumbers[j]-1 do begin
      ;        phase_all[0,*,*,obs_i]=atan((*cal_array[obs_i].gain[0]),/phase)
      ;        phase_all[1,*,*,obs_i]=atan((*cal_array[obs_i].gain[1]),/phase)
      ;      endfor
      ;      wh_phases=where(phase_all EQ 0,n_phases_zero)
      ;      if n_phases_zero GT 0 then phase_all[wh_phases] = !VALUES.F_NAN
      ;      phase_mean=mean(phase_all,dimension=4,/NAN)
      ;
      ;      for freq_i=0,383 do begin
      ;        for i=0, parsednumbers[j]-1 do begin
      ;          ;(*cal_array[i].gain[0])[freq_i,*]=abs((*cal_array[i].gain[0])[freq_i,*]); *exp(Complex(0,1)*phase_mean[0,freq_i,*])
      ;          ;(*cal_array[i].gain[1])[freq_i,*]=abs((*cal_array[i].gain[1])[freq_i,*]); *exp(Complex(0,1)*phase_mean[1,freq_i,*])
      ;          ;(*cal_array[i].gain[0])[freq_i,*]= exp(Complex(0,1)*phase_mean[0,freq_i,*])
      ;          ;(*cal_array[i].gain[1])[freq_i,*]= exp(Complex(0,1)*phase_mean[1,freq_i,*])
      ;        endfor
      ;      endfor
      ...
      If keyword_set(split_jump) then begin
        ...
        cal_polyfit_phase=vis_cal_polyfit_pointing(cal_array,obs_array,parsednumbers[j],pointing_num[j],degree=degree,phase_degree=1,$
          file_path=dir_name+'/',cal_cable_reflection_mode_fit=1,cal_cable_reflection_fit=cal_cable_reflection_fit,cal_mode_fit=cal_mode_fit,pointingmode=pointingmode)
        ...
        if ~keyword_set(pointingmode) then begin
          ...
          for obs_i=0, parsednumbers[j]-1 do begin
            ...
            poly_input_gains[256:383,tile_use,0,obs_i]=abs((*cal_polyfit_post[obs_i].gain[0])[256:383,tile_use])
            poly_input_gains[0:255,tile_use,0,obs_i]=abs((*cal_polyfit_pre[obs_i].gain[0])[0:255,tile_use])
            
            poly_input_gains[256:383,tile_use,1,obs_i]=abs((*cal_polyfit_post[obs_i].gain[1])[256:383,tile_use])
            poly_input_gains[0:255,tile_use,1,obs_i]=abs((*cal_polyfit_pre[obs_i].gain[1])[0:255,tile_use])
            
            poly_input_gains[*,tile_use,0,obs_i]=poly_input_gains[*,tile_use,0,obs_i]*exp(Complex(0,1)*atan(((*cal_polyfit_phase[obs_i].gain[0])[*,tile_use]),/phase))
            poly_input_gains[*,tile_use,1,obs_i]=poly_input_gains[*,tile_use,1,obs_i]*exp(Complex(0,1)*atan(((*cal_polyfit_phase[obs_i].gain[1])[*,tile_use]),/phase))
            
            If keyword_set(poly_saved_file) then begin
              ;gain_fit_restored=getvar_savefile('/nfs/eor-00/h1/nbarry/longrun_polyfit.sav','gain_fit') ;pol x day x freq x tile x pointing
              poly_input_gains[*,tile_use,0,obs_i]=gain_fit_restored[0,0,*,tile_use,j]*exp(Complex(0,1)*atan(((*cal_polyfit_phase[obs_i].gain[0])[*,tile_use]),/phase))
              poly_input_gains[*,tile_use,1,obs_i]=gain_fit_restored[1,0,*,tile_use,j]*exp(Complex(0,1)*atan(((*cal_polyfit_phase[obs_i].gain[1])[*,tile_use]),/phase))
            endif
          ...
          if keyword_set(compare) then begin
            ...
            compare_ratio_array[where(cross_poly EQ 1)]=1.
            compare_ratio=FLTARR(128,2)
            for pol_j=0,1 do begin
              for tile_i=0,127 do begin
                ;resistant_mean,compare_ratio_array[*,tile_i,pol_j,0],2,res_mean
                ;compare_ratio[tile_i,pol_j]=res_mean
                compare_ratio[tile_i,pol_j]=mean(compare_ratio_array[where(compare_ratio_array[*,tile_i,pol_j,0] NE 1),tile_i,pol_j,0])
              endfor
            endfor
          endif
        ;  save, compare_ratio, filename='/nfs/eor-00/h1/nbarry/compare_ratio_v2_'+strtrim(pointing_num[j],2)+'.sav'
        ;stop
        ...
        ;           ;resistant_mean,compare_ratio_array[*,tile_i,pol_j,0],2,res_mean
        ;           ;compare_ratio[tile_i,pol_j]=res_mean
        ;           compare_ratio[tile_i,pol_j]=mean(compare_ratio_array[*,tile_i,pol_j,0])
        ;         endfor
        ;      endfor
        ;    endif
        ...
        cal_array_input=cal_array
        if keyword_set(compare) then begin
          for obs_j=0,parsednumbers[j]-1 do begin
            for pol_j=0,1 do begin
              (cal_array_input[obs_j].gain[pol_j])=Pointer_copy(cal_array[obs_j].gain[pol_j])
              
              if keyword_set(phase_transfer) then begin
              
                for freq_i=0,383 do begin
                ;(*cal_array_input[obs_j].gain[pol_j])[freq_i,tile_use]=((*cal_array_input[obs_j].gain[pol_j])[freq_i,tile_use]/compare_ratio[tile_use,pol_j])*$
                ;  exp(Complex(0,1)*(reform((*phase_transfer_ptr[obs_j])[pol_j,freq_i,tile_use])))
                ;  (*cal_array_input[obs_j].gain[pol_j])[freq_i,tile_use]=(*cal_array_input[obs_j].gain[pol_j])[freq_i,tile_use]*$
                ;    exp(Complex(0,1)*(reform((*phase_transfer_ptr[obs_j])[pol_j,freq_i,tile_use])))
                endfor
        ...
        ;        stop
        ;cal_polyfit_total=vis_cal_modefit_only_pointing(poly_input_gains,cal_array_input,obs_array,parsednumbers[j],pointing_num[j],degree=1,phase_degree=1,$
        ...
        cal_polyfit_total=vis_cal_modefit_only_pointing(poly_input_gains,cal_array_input,obs_array,parsednumbers[j],pointing_num[j],degree=1,phase_degree=1,$
          file_path=dir_name+'/',cal_cable_reflection_mode_fit=1,cal_cable_reflection_both=1,pointingmode=pointingmode,mode_input=mode_input_arr);,orig_mode_flagging=1)
        ...
        ;  file_path=dir_name+'/',cal_cable_reflection_fit=150,cal_cable_reflection_mode_fit=1)
        ;endif
