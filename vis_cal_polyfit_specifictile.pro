FUNCTION vis_cal_polyfit_specifictile,cal,obs,degree=degree,phase_degree=phase_degree,$
    cal_step_fit=cal_step_fit,cal_neighbor_freq_flag=cal_neighbor_freq_flag,tile_index=tile_index,$
...
IF Keyword_Set(cal_mode_fit) THEN BEGIN
    ...
    CASE 1 OF
        ...
        Keyword_Set(cal_cable_reflection_fit): BEGIN
            ...
            actual_cable_len=cable_len[tile_index]
            actual_cable_vf=cable_vf[tile_index]
            actual_tile_ref_flag=tile_ref_flag[tile_index]
            cable_len=FLTARR(cal.n_tile)
            cable_vf=FLTARR(cal.n_tile)
            tile_ref_flag=FLTARR(cal.n_tile)
            cable_len[*]=actual_cable_len
            cable_vf[*]=actual_cable_vf
            tile_ref_flag[*]=actual_tile_ref_flag
