FUNCTION vis_calibrate,vis_ptr,cal,obs,status_str,psf,params,jones,flag_ptr=flag_ptr,model_uv_arr=model_uv_arr,$
    ...
    saved_run_polysplit_onemode=saved_run_polysplit_onemode,saved_run_std_test_polyquad=saved_run_std_test_polyquad,_Extra=extra
  t0_0=Systime(1)
  error=0
  timing=-1
  heap_gc
  ...
  IF Keyword_Set(calibration_polyfit) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      If keyword_set(saved_run_polysplit_onemode) then begin
        ...
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_std_test_twopolyquad_fancymodeobs_Ianscaled_crossamp/'+obs.obsname+'_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])=(*cal.gain[0])
        (*cal_polyfit.gain[1])=(*cal.gain[1])
      ...
      If keyword_set(saved_run_std_test_polyquad) then begin
        ...
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_std_test_twopolyquad_fancymodeobs_150only/'+obs.obsname+'_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])=(*cal.gain[0])
        (*cal_polyfit.gain[1])=(*cal.gain[1])
