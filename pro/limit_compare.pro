pro limit_compare
  ...
  basefile[3]=['/nfs/mwa-04/r1/EoRuvfits/analysis/fhd_nb_2013longrun_autocal/ps_bn/Combined_obs_'+cut[3]+'_cubeXX__even_odd_joint_'+win[3]+chans[3]]
  basefile[2]=['/nfs/mwa-04/r1/EoRuvfits/analysis/fhd_nb_2013longrun_autocal/ps_bn/Combined_obs_'+cut[2]+'_cubeXX__even_odd_joint_'+win[2]+chans[2]]
  ...
  basefile[1]=['/nfs/mwa-04/r1/EoRuvfits/analysis/fhd_nb_2013longrun_autocal/ps_bh/Combined_obs_'+cut[1]+'_cubeXX__even_odd_joint_'+win[1]+chans[1]]
  ...
  *ranges[3]=[6,7,8,9,10,11,21,22,23,24,25,26,36,37,38,39,40,41,50,51,52]
  ...
  ;legend_title=['z=7.1','z=6.8','z=6.5','z=7']
  legend_title=['tk 10-127','bh 0-95','bn 0-95','bn 10-127']
  ...
  for j=0,N_elements(pols)-1 do begin
    for cube_i=0,n_cubes-1 do begin
      ...
      ;if (cube_i EQ 0) then cgPS_Open,'/nfs/mwa-00/h1/nbarry/'+pols[j]+'_limit_comparison.png',/quiet,/nomatch
      if cube_i EQ 0 then cgplot, k,abs(limits),/xlog,/ylog,psym=10, xrange=[.15,1.7], yrange=[10^2.,10^7.],ytitle='$\Delta$$\up2$ (mK$\up2$)', $
        xtitle='k (h Mpc$\up-1$)', title='UW preliminary EoR limit', charsize =1.25, color=color_array[cube_i],thick=5 else $
          cgoplot, k,abs(limits),/xlog,/ylog,psym=10, xrange=[.15,1.7],color=color_array[cube_i],thick=5,charsize =1.25,xtitle='k (h Mpc$\up-1$)', title='UW preliminary EoR limit',ytitle='$\Delta$$\up2$ (mK$\up2$)'
      if cube_i EQ 3 then  cgoplot, k,dsigma,/xlog,/ylog,psym=10,linestyle=2, color=color_array[cube_i],thick=5
      
      
    endfor
    cglegend, title=legend_title,color=color_array, location=[.2,.85], charsize=1, thick=3
