FUNCTION vis_calibrate,vis_ptr,cal,obs,status_str,psf,params,jones,flag_ptr=flag_ptr,model_uv_arr=model_uv_arr,$
    ...
    calibration_auto_initialize=calibration_auto_initialize,calibration_auto_fit=calibration_auto_fit,saved_run_poly_split=saved_run_poly_split,$
    saved_run_poly_unsplit=saved_run_poly_unsplit,saved_run_phasetransfer_quad=saved_run_phasetransfer_quad,$
    saved_run_polysplit_onemode=saved_run_polysplit_onemode,saved_run_nodig_quad_polyscaled=saved_run_nodig_quad_polyscaled,_Extra=extra
  t0_0=Systime(1)
  error=0
  timing=-1
  heap_gc
...
IF Keyword_Set(bandpass_calibrate) THEN BEGIN
  ...
  IF Keyword_Set(calibration_polyfit) THEN BEGIN
    ...
    ENDIF ELSE BEGIN
      ...
      If keyword_set(saved_run_poly_split) then begin
        ;saved_run location
        print, 'Saved poly run activated!'
        ...
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_pointing_nodigjump_v2_plusmodepointing_frombp/'+obs.obsname+'pre_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])[0:255,*]=(*cal.gain[0])[0:255,*]
        (*cal_polyfit.gain[1])[0:255,*]=(*cal.gain[1])[0:255,*]
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_pointing_nodigjump_v2_plusmodepointing_frombp/'+obs.obsname+'post_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])[256:383,*]=(*cal.gain[0])[256:383,*]
        (*cal_polyfit.gain[1])[256:383,*]=(*cal.gain[1])[256:383,*]
        
        if keyword_set(cal_store) then cal=cal_store
        
      endif
      
      If keyword_set(saved_run_poly_unsplit) then begin
        ;saved_run location
        print, 'Saved poly run reg activated!'
        
        if keyword_set(cal) then cal_store=cal
        
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_unscaledautos_phasetransfer_polyratio/forinput/'+obs.obsname+'_cal.sav
        restore,filename_poly
      ...
      If keyword_set(saved_run_phasetransfer_quad) then begin
        ;saved_run location
        print, 'Saved poly run saved_run_phasetransfer_quad activated!'
        
        if keyword_set(cal) then cal_store=cal
        
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_polyratio_quadsplit/forinput/'+obs.obsname+'_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])=(*cal.gain[0])
        (*cal_polyfit.gain[1])=(*cal.gain[1])
      ...
      If keyword_set(saved_run_polysplit_onemode) then begin
        ;saved_run location
        print, 'Saved poly run saved_run_polysplit_onemode activated!'
        
        if keyword_set(cal) then cal_store=cal
        
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_polysplit_onemode_catchup/'+obs.obsname+'_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])=(*cal.gain[0])
        (*cal_polyfit.gain[1])=(*cal.gain[1])
      ...
      If keyword_set(saved_run_nodig_quad_polyscaled) then begin
        ;saved_run location
        print, 'Saved poly run saved_run_nodig_quad_polyscaled activated!'
        
        if keyword_set(cal) then cal_store=cal
        
        ;filename_poly='/nfs/eor-00/h1/nbarry/Aug23_nodig_quad_polyscaled/forinput/'+obs.obsname+'_cal.sav
        filename_poly='/nfs/eor-00/h1/nbarry/Aug23_onequad_polyscaled_90150/forinput/'+obs.obsname+'_cal.sav
        restore,filename_poly
        (*cal_polyfit.gain[0])=(*cal.gain[0])
        (*cal_polyfit.gain[1])=(*cal.gain[1])
