pro component_plotter
  ...
  for day_i=0,0 do begin
    ...
    if day_i EQ 0 then dir = '/fred/oz048/MWA/CODE/FHD/fhd_nb_decon_aug23/deconvolution/'
    ...
    if keyword_set(fits_files) then begin
      for obs_i=0, N_elements(obs_id) -1 do begin
        ...
        hdr=["COMMENT   = Field: EoR0, Date: Aug23, MHz: " + strtrim(freq[0],2)+", ObsID: " +obs_id[obs_i],'']
        
        mwrfits, component_struct,dir+obs_id[obs_i]+'_highEoR0.fits',hdr,/create
      endfor
    endif
    
    if keyword_set(plot_components) then begin
