...
FUNCTION vis_calibrate_subroutine_kmode,vis_ptr,vis_model_ptr,vis_weight_ptr,obs,params,cal,preserve_visibilities=preserve_visibilities,$
    calib_freq_func=calib_freq_func,calibration_weights=calibration_weights,cal_by_k=cal_by_k,_Extra=extra
  ...
  ;Testing
  if keyword_set(cal_by_k) then begin
  	vis_ptr_orig = Pointer_copy(vis_ptr)
  	vis_weight_ptr_use_orig = Pointer_copy(vis_weight_ptr_use)
  	calibration_by_k, vis_ptr, vis_model_ptr, vis_weight_ptr_use, obs, params, _Extra=extra
  endif
...
;    endelse
  ...
  stop
        ;Testing   
  if keyword_set(cal_by_k) then begin
  	vis_cal=vis_calibration_apply(vis_ptr,cal_return)
  	for pol_i=0, n_pol-1 do begin
  		
  	  gain_arr_final = (FFT(*vis_cal[pol_i], /INVERSE, DIMENSION=1)/*vis_ptr_orig[pol_i]) 
  	  zeros = where(*vis_weight_ptr_use[pol_i] EQ 0, n_count)
  	  gain_arr_final[zeros]=0
  	  
  	  vis_weight_use=0>Reform(*vis_weight_ptr_use[pol_i],n_freq,n_baselines,n_time)<1
      gain_arr_final=Reform(gain_arr_final,n_freq,n_baselines,n_time)

      gain_arr_final=Total(Temporary(gain_arr_final)*vis_weight_use,3)
      weight=Total(Temporary(vis_weight_use),3)
      gain_arr_final*=weight_invert(weight)

  	  
  	  gain_arr=complex(FLTARR(384,128));*cal.gain[pol_i]
  	  gain_arr[*,*]=1.
  	  FOR fii=0L,n_freq_use-1 DO BEGIN
        fi=freq_use[fii]
        gain_curr=Reform(gain_arr[fi,tile_use])
        ...
        FOR i=0L,(max_cal_iter-1)>1 DO BEGIN
            ...
            vis_model_matrix=Conj(gain_curr[B_ind])

                FOR tile_i=0L,n_tile_use-1 DO IF n_arr[tile_i] GE min_cal_solutions THEN $
                    gain_new[tile_i]=LA_Least_Squares(vis_model_matrix[*A_ind_arr[tile_i]],gain_arr_final[fi,*A_ind_arr[tile_i]],method=2, double=double_precision)
