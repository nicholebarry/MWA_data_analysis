FUNCTION vis_calibrate_subroutine_tau_g2,vis_ptr,vis_model_ptr,vis_weight_ptr,obs,params,cal,preserve_visibilities=preserve_visibilities,$
  ...
  FOR pol_i=0,n_pol-1 DO BEGIN
    ...
    ;vis_gain=gain_arr_tau[inds_A]*Conj(gain_arr_tau[inds_B])
    vis_data_save = vis_avg
    vis_model_save = vis_model
    ;vis_avg=fft(fft(vis_model,dimension=1)*(vis_gain),dimension=1,/inverse)
    ;vis_avg=vis_model*vis_gain
  ...
  window_expand = rebin(reform(window,n_freq,1,1), n_freq, n_baselines, /sample)
    ...
    ;slope = (vis_avg[freq_use[N_elements(freq_use)-1],*] - vis_avg[freq_use[0],*])/(freq_use[N_elements(freq_use)-1]-freq_use[0])
    ;vis_avg[freq_use,*] = vis_avg[freq_use,*] - (rebin_complex(slope,N_elements(freq_use),n_baselines) * rebin((FINDGEN(n_freq))[freq_use], N_elements(freq_use),n_baselines)$
    ;   + rebin_complex(vis_avg[freq_use[0],*],N_elements(freq_use),n_baselines))
       
    ;slope = (vis_model[freq_use[N_elements(freq_use)-1],*] - vis_model[freq_use[0],*])/(freq_use[N_elements(freq_use)-1]-freq_use[0])
    ;vis_model[freq_use,*] = vis_model[freq_use,*] - (rebin_complex(slope,N_elements(freq_use),n_baselines) * rebin((FINDGEN(n_freq))[freq_use], N_elements(freq_use),n_baselines)$
    ;   + rebin_complex(vis_model[freq_use[0],*],N_elements(freq_use),n_baselines))
    
    vis_avg = fft(vis_avg*window_expand, dimension=1)
    vis_model = fft(vis_model*window_expand, dimension=1)
    
    ;freq_use = FINDGEN(n_freq)
    ;n_freq_use=n_freq
    ...
    FOR fii=0L,n_freq_use-1 DO BEGIN
      ...
      ;weight2=Reform(total(weight[*,baseline_use],1)) & weight2=[weight2,weight2]
      ...
      FOR i=0L,(max_cal_iter-1)>1 DO BEGIN
        ...
        gain_curr*=Conj(gain_curr[ref_tile_use])/Abs(gain_curr[ref_tile_use]) ;TEMP
