...
FUNCTION vis_calibrate_subroutine_tau,vis_ptr,vis_model_ptr,vis_weight_ptr,obs,params,cal,preserve_visibilities=preserve_visibilities,$
  ...
  ;for freq_i=0, n_freq-1 do *kperp_mask_inds[freq_i] = where(kperp_mask GT 0,n_count)
    ...
    FOR i=0L,(max_cal_iter-1)>1 DO BEGIN
      ...
      gain_new=Complexarr(n_freq_use,n_tile_use)
      ...
      gain_new = fft(gain_new,dimension=1,/inverse) ;gain as a func of tau
      ...
      convergence[*,tile_use]=Abs(gain_curr-gain_old)*weight_invert(Abs(gain_old))
      convergence_phase[*,tile_use]=(phunwrap(atan(gain_curr,/phase))-phunwrap(atan(gain_old,/phase)));*weight_invert(phunwrap(Atan(gain_old,/phase)))
